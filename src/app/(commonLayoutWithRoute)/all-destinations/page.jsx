  ...
  const applyFilters = () => {
    ...
    const filtered = destinations.filter((destination) => {
      const withinPriceRange =
        destination.packagePrice >= priceRange[0] &&
        destination.packagePrice <= priceRange[1];
      const withinRatingRange =
        destination.ratingAverage >= ratingRange[0] &&
        destination.ratingAverage <= ratingRange[1];
      const withinCategory =
        categories.length === 0 || categories.includes(destination.category);
