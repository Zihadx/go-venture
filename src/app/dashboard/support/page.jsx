...
export default function SupportTicketsPage() {
  ...
  const onCreateTicket = async (values) => {
    await createTicket({ subject: values.subject, message: values.message, customer: { name: user.name, email: user.email } });
    reset();
    load();
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-8">
      <div className="lg:col-span-1 space-y-4">
        {!isStaff && (
          <form onSubmit={handleSubmit(onCreateTicket)} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3">
            <h3 className="font-bold text-gray-800">New ticket</h3>
            <RHFTextField name="subject" control={control} size="small" fullWidth label="Subject" />
            <RHFTextField name="message" control={control} size="small" fullWidth multiline rows={3} label="How can we help?" />
