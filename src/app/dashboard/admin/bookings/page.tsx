...
import { useBookingsQuery, useBookingDetailQuery, useUpdateBookingStatus } from "@/hooks/queries/useBookingsQuery";
...
export default function BookingsManagementPage() {
  ...
  const { data, isLoading } = useBookingsQuery({ page, pageSize, search, status, paymentStatus });
  const { data: selected } = useBookingDetailQuery(selectedId) as { data: BookingRow | null | undefined };
  ...
  const columns: DataTableColumn<BookingRow>[] = [
    { key: "id", header: "Booking", render: (b) => <span className="font-semibold text-gray-700">{b.id}</span> },
    {
    ...
    { key: "destination", header: "Destination", render: (b) => <span className="text-gray-600">{b.destination}</span> },
    { key: "travelDate", header: "Travel date", render: (b) => <span className="text-gray-500">{formatDate(b.travelDate)}</span> },
    { key: "amount", header: "Amount", align: "right", render: (b) => <span className="font-medium text-gray-700">{formatCurrency(b.amount)}</span> },
    { key: "paymentStatus", header: "Payment", render: (b) => <StatusChip status={b.paymentStatus} /> },
    { key: "status", header: "Status", render: (b) => <StatusChip status={b.status} /> },
    { key: "bookedAt", header: "Booked", render: (b) => <span className="text-gray-400">{timeAgo(b.bookedAt)}</span> },
  ];
        ...
        toolbar={
          <>
            ...
            <Select size="small" value={status} onChange={(e) => { setPage(0); setStatus(e.target.value); }} className="w-full md:w-52">
              {STATUS_FILTERS.map((s) => <MenuItem key={s} value={s} className="capitalize">{s === "all" ? "All statuses" : s.replace("_", " ")}</MenuItem>)}
            </Select>
            <Select size="small" value={paymentStatus} onChange={(e) => { setPage(0); setPaymentStatus(e.target.value); }} className="w-full md:w-44">
              {PAYMENT_FILTERS.map((s) => <MenuItem key={s} value={s} className="capitalize">{s === "all" ? "All payments" : s}</MenuItem>)}
            </Select>
            <button
          ...
          <div className="w-[380px] sm:w-[440px] h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><p className="text-xs font-bold uppercase text-gray-400 mb-1">Destination</p><p className="text-gray-700">{selected.destination}</p></div>
                <div><p className="text-xs font-bold uppercase text-gray-400 mb-1">Package</p><p className="text-gray-700">{selected.package}</p></div>
                <div><p className="text-xs font-bold uppercase text-gray-400 mb-1">Travel date</p><p className="text-gray-700">{formatDate(selected.travelDate)}</p></div>
                <div><p className="text-xs font-bold uppercase text-gray-400 mb-1">Travelers</p><p className="text-gray-700">{selected.travelers}</p></div>
                <div><p className="text-xs font-bold uppercase text-gray-400 mb-1">Amount</p><p className="text-gray-700 font-semibold">{formatCurrency(selected.amount)}</p></div>
                <div><p className="text-xs font-bold uppercase text-gray-400 mb-1">Assigned agent</p><p className="text-gray-700">{selected.agent}</p></div>
              </div>
            ...
            {NEXT_STATUS[selected.status] && (
              <div className="p-4 border-t flex gap-2">
                ...
                {selected.status === "confirmed" && (
                  <button disabled={updateStatus.isPending} onClick={() => updateStatus.mutate({ id: selected.id, status: "ongoing" })} className="flex-1 button-primary disabled:opacity-50">
                    Mark trip as ongoing
                  </button>
                )}
                {selected.status === "ongoing" && (
                  <button disabled={updateStatus.isPending} onClick={() => updateStatus.mutate({ id: selected.id, status: "completed" })} className="flex-1 button-primary disabled:opacity-50">
                    Mark trip completed
                  </button>
                )}
                {selected.status === "refund_requested" && (
                  <button disabled={updateStatus.isPending} onClick={() => updateStatus.mutate({ id: selected.id, status: "refunded" })} className="flex-1 button-primary disabled:opacity-50">
                    Approve refund
                  </button>
                )}
              </div>
