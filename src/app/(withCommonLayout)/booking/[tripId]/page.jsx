...
import { validateCoupon, redeemCoupon } from "@/services/billing.service";
import useCurrentUser from "@/hooks/useCurrentUser";
import { formatCurrency } from "@/utils/format";
import { travelerStepSchema } from "@/schemas/booking.schema";
...
export default function BookingWizardPage() {
  ...
  const {
    control: travelerControl,
    getValues: getTravelerValues,
    formState: { isValid: travelersValid },
  } = useForm({
    resolver: zodResolver(travelerStepSchema),
    mode: "onChange",
    defaultValues: { travelers: [emptyTraveler()], emergencyContact: { name: "", phone: "" } },
  });
  const { fields: travelerFields, replace: replaceTravelers } = useFieldArray({ control: travelerControl, name: "travelers" });
  ...
  useEffect(() => {
    const current = getTravelerValues("travelers") || [];
    const next = [...current];
    while (next.length < travelerCount) next.push(emptyTraveler());
    replaceTravelers(next.slice(0, travelerCount));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [travelerCount]);

  const pricing = useMemo(() => {
    if (!trip) return { base: 0, addOnsTotal: 0, discount: 0, tax: 0, total: 0 };
    ...
    const discount = appliedCoupon ? Math.round((base + addOnsTotal) * (appliedCoupon.discountPercent / 100)) : 0;
    const taxable = Math.max(base + addOnsTotal - discount, 0);
    const tax = Math.round(taxable * 0.05);
    return { base, addOnsTotal, discount, tax, total: taxable + tax };
  }, [trip, travelerCount, addOns, appliedCoupon]);
  ...
  const canProceed = () => {
    ...
    if (step === 1) return travelersValid;
    return true;
  };
  ...
  const handleApplyCoupon = async () => {
    ...
    if (!coupon) {
      setCouponError("That code is invalid, expired, or fully redeemed.");
    ...
    <div className="custom-container py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              {travelerFields.map((field, i) => (
                <div key={field.id} className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 rounded-lg bg-gray-50">
                  <RHFTextField
                  ...
                  />
                  <RHFTextField name={`travelers.${i}.age`} control={travelerControl} label="Age" type="number" size="small" />
                  <RHFTextField name={`travelers.${i}.passport`} control={travelerControl} label="Passport / NID (optional)" size="small" />
                </div>
              ))}
              <Divider />
              <h3 className="font-semibold text-gray-700">Emergency contact</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <RHFTextField name="emergencyContact.name" control={travelerControl} label="Contact name" size="small" />
                <RHFTextField name="emergencyContact.phone" control={travelerControl} label="Contact phone" size="small" />
              </div>
            </div>
          )}
          ...
          {step === 3 && (() => {
            const { travelers: reviewTravelers, emergencyContact: reviewContact } = getTravelerValues();
            return (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><p className="text-xs uppercase text-gray-400">Emergency contact</p><p className="text-gray-700 font-medium">{reviewContact.name} · {reviewContact.phone}</p></div>
              </div>
              <Divider />
              <div>
                <p className="text-xs uppercase text-gray-400 mb-2">Travelers</p>
                <ul className="text-sm text-gray-600 space-y-1">
                  {reviewTravelers.map((t, i) => <li key={i}>{i + 1}. {t.name} — Age {t.age}{t.passport ? ` — ${t.passport}` : ""}</li>)}
                </ul>
              </div>
              <Divider />
              <div>
              ...
              <Divider />
              <div>
                <p className="text-xs uppercase text-gray-400 mb-2">Promo code</p>
                {appliedCoupon ? (
                  <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                    <p className="text-sm text-green-700 font-semibold">{appliedCoupon.code} applied — {appliedCoupon.discountPercent}% off</p>
                    <button onClick={() => { setAppliedCoupon(null); setCouponInput(""); }} className="text-xs text-green-700 underline">Remove</button>
                  </div>
                ) : (
                  <div className="flex gap-2">
                    <TextField
                      size="small" placeholder="Enter promo code" value={couponInput}
                    ...
                    <button
                      ...
                      className="rounded-md border border-primary text-primary px-4 text-sm font-semibold hover:bg-primary/5 disabled:opacity-50"
                    >
        ...
        <aside className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 h-fit sticky top-24">
          ...
          <div className="space-y-2 text-sm">
            ...
            {pricing.discount > 0 && (
              <div className="flex justify-between text-green-600"><span>Discount {appliedCoupon ? `(${appliedCoupon.code})` : ""}</span><span>-{formatCurrency(pricing.discount)}</span></div>
            )}
