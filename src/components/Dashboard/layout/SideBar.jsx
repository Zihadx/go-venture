  ...
  return (
    <div
      className={`h-screen transition-all duration-300 overflow-y-auto scrollbar-hide ${
        isOpen ? "w-64" : "w-20"
      } bg-white border-r relative flex flex-col`}
    >
      ...
      {/* Logo */}
      <Link href="/" className="flex items-center gap-2 p-4 border-b border-gray-100">
        ...
        <div className={`${isOpen ? "block" : "hidden"} transition-all duration-300 leading-tight`}>
          <h1 className="text-base font-bold text-secondary">Go-Venture</h1>
              ...
              {items.map((item) => {
                ...
                const content = (
                  ...
                  >
                    ...
                    {isOpen && isSoon && (
                      <span className="text-[9px] font-bold uppercase tracking-wide bg-gray-100 text-gray-400 rounded px-1.5 py-0.5">
                        Soon
                      </span>
                    )}
                  </div>
