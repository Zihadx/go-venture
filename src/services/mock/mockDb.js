...
// Same 12-month bucketing as buildMonthlySeries, scoped to one agent's bookings.
export function buildAgentMonthlySeries(agentName) {
  const months = [];
  const now = new Date();
  for (let i = 11; i >= 0; i--) {
    const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
  ...
  const bookingsCount = months.map(() => 0);
  const commission = months.map(() => 0);

  MOCK_BOOKINGS.filter((b) => b.agent === agentName).forEach((b) => {
    const d = new Date(b.bookedAt);
    const idx = months.findIndex((m) => m.key === `${d.getFullYear()}-${d.getMonth()}`);
    if (idx === -1) return;
    bookingsCount[idx] += 1;
    if (b.status !== "cancelled") commission[idx] += Math.round(b.amount * 0.08);
  });

  return { labels: months.map((m) => m.label), bookingsCount, commission };
}

export function buildAgentCustomers(agentName) {
  const seen = new Map();
  MOCK_BOOKINGS.filter((b) => b.agent === agentName).forEach((b) => {
    const key = b.customer.email;
    const existing = seen.get(key) || { name: b.customer.name, email: b.customer.email, bookings: 0, spend: 0 };
    existing.bookings += 1;
    if (b.status !== "cancelled") existing.spend += b.amount;
    seen.set(key, existing);
  });
  return Array.from(seen.values()).sort((a, b) => b.spend - a.spend);
}

// Lightweight mock task list — a real backend would drive this from a
// tasks/CRM collection; kept deterministic here via the same seeded rng.
const TASK_TEMPLATES = [
  { title: "Follow up on quote for {customer}", type: "quote" },
  { title: "Confirm travel documents for {customer}", type: "documents" },
  { title: "Send itinerary to {customer}", type: "itinerary" },
  { title: "Call {customer} about add-on upgrade", type: "call" },
  { title: "Chase payment for {customer}", type: "payment" },
];

export function buildAgentTasks(agentName) {
  const customers = buildAgentCustomers(agentName).slice(0, 6);
  return customers.map((c, i) => {
    const template = TASK_TEMPLATES[i % TASK_TEMPLATES.length];
    return {
      id: `TASK-${agentName.slice(0, 2).toUpperCase()}-${i + 1}`,
      title: template.title.replace("{customer}", c.name),
      type: template.type,
      dueInDays: int(0, 6),
      done: i % 4 === 0,
    };
